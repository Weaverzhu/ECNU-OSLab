...
编写一个程序，访问空指针，原代码见 \cw{./xv6 VM Layout/user/nulldereference.c}
...
修改 \cw{./xv6 VM Layout/user/makefile.mk}，添加我们编写的新程序
...
\\ 在 \cw{./xv6 VM Layout/include/types.h} 中我们看到 NULL 被定义为 0。因此一个空指针会访问到 User text 部分，这部分的页表 flag 上有 \cw{Present} 因此会被认为是一个合法的内存访问。现在我们希望把前两页空出来，这样当前两页将没有 \cw{Present} flag，访问的时候 kernel 就会帮我们处理错误。
...
\begin{itemize}
    \item \cw{./xv6 VM Layout/kernel/kalloc.c kalloc} 该函数分配一个 4096 长度的物理内存空间
    \item \cw{./xv6 VM Layout/kernel/vm.c walkpgdir} 返回页表地址
    \item \cw{./xv6 Vm Layout/kernel/vm.c allocuvm} 为某个进程的页表分配新的空间，从 \cw{oldsz} 增长到 \cw{newsz}
    \item \cw{./xv6 VM Layout/kernel/vm.c mappages} 该函数把会根据新分配的物理空间（4096长度）建立页表。也就是说会建立参数 \cw{la} (虚拟地址) 到 \cw{pa} (物理地址) 之间的映射。我们可以看到获取 \cw{la} 的地址后会检查是否该地址已经被其他物理地址占用了（\cw{panic("remap")}），否则就把 flag 加上 \cw{Present} 一起添加。由之前的定义，相邻 page table directory 本来就有相邻的地址，如果这个 page table 满了自动就会应用到下一个 page table 中。（其实也可以看作是 $2^{20}$ 个连续页表）
\end{itemize}
...
用户程序的装在位置在 \cw{./xv6 Vm Layout/user/makefile.mk} 需要被更改
...
\cw{./xv6 VM Layout/kernel/vm.c copyuvm}
...
\cw{./xv6 Vm Layout/user/init.c} 当中被找到
...
\cw{./xv6 Vm Layout/kernel/proc.h}
