...
// should update all threads' size!
int
growproc(int n)
{
  uint sz;
  ...
  // will update all the threads which are belong to proc
  for (p=ptable.proc; p<&ptable.proc[NPROC]; ++p) {
...
hich is passed the given argument arg and uses a fake return PC (0xffffffff). 
...
// This call waits for a child thread that 
// shares the address space with the calling process.
